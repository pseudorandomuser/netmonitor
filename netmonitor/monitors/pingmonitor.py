...
class PingMonitor(Monitor):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._ping_path: str = self._ping_bin if self._ping_bin else '/sbin/ping'
        self._host_list: List[str] = self.kwargs.get('host_list', [])

    def _ping(self, host_addr: str, attempts: int = 1, timeout: int = 1) -> Tuple[bool, float]:
        ping_proc: subprocess.Popen = subprocess.Popen([
            self._ping_path, '-c', str(attempts), '-t', str(timeout), host_addr
        ], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ping_output: str = ping_proc.communicate()[0].decode('UTF-8')
        ping_search: Optional[re.Match] = re.search('time=(.*) ms', ping_output)
    ...
    def _tick(self) -> None:
        for host in self._host_list:
            ...
            timestamp: str = current_date.strftime('%A, %d. %B %Y at %H:%M:%S')
            previous_state: Optional[bool] = self._host_states.get(host)
            current_state, current_ping = self._ping(host)
            if previous_state != current_state:
                self.notifier.notify(f'Host "{hostname}" ({host}) is {f"up, ping {current_ping} ms" if current_state else "down"}, {timestamp}.')
